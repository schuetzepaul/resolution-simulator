...
int main(int argc, char* argv[]) {
  ...
  TProfile *resolution = new TProfile("resolution"," ",32,-2.5,157.5);

  TCanvas *c2 = new TCanvas("c2","kink_resolution",700,700);
  TProfile *kink_resolution = new TProfile("kink_resolution"," ",32,-2.5,157.5);
  ...
  double DUT_DIST_up = 50;
  double DUT_DIST_down = 50;

  // Beam energy 2 GeV electrons/positrons at DESY, highest rate
  double BEAM = 2.0;

  // Random target with eps=1 
  double dut_x0 = 5.612/5.612;
  double dut_size = 5.;
  ...
  for(double DIST_down = 10.; DIST_down < 155.; DIST_down += 5.) {
    ...
    double dut_position = 2*DIST_up + DUT_DIST_up;
    plane dut = plane::unknown(dut_position, dut_size);
    ...
    LOG(logRESULT) << "Track resolution at DUT, dzdown = " << DIST_down << " : " << mytel.getResolution(3);
    resolution->Fill(DIST_down,mytel.getResolution(3),1);

    LOG(logRESULT) << "Kink resolution  at DUT, dzdown = " << DIST_down << " : " << mytel.getKinkResolution(3);
  ...
  c1->cd();
  resolution->SetTitle("DATURA Track Resolution at DUT;dz of downstream planes [mm];resolution at DUT #left[#mum#right]");
